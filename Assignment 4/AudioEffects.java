...
public class AudioEffects
{
    ...
    public static double [] audioReverse(double [] audio)
    {
        ...
        double [] result = new double[audio.length];
        // get data last to first from the audio array and store in result array position i
        for (int i = 0; i < audio.length; i++)
            result[i] = audio[audio.length - 1 - i];
        return result;  
    }
    ...
    {
        // TBD
        // declare a variable to hold the length of the result array
        int newLength = audio.length * loops;
        // declare the result array
        double [] result = new double[newLength];
        // initialize a counter to keep track of the position in result array
        int c = 0;
        // while result array is full
        while (c < newLength)
            // add 1 loop of the audio array
            for (int i = 0; i < audio.length; i++)
            {
                // add the audio array data to the result array
                result[c] = audio[i];
    ...
    {
        // figure out which, if any, audio array is shorter 
        int length;
        if (audio1.length < audio2.length)
            length = audio1.length;
        else 
            length = audio2.length;

        // declare the result array
        double [] result = new double[length];
        // merge the data from the audio files while averaging the samples
    ...
    public static double [] audioCut(double [] audio, double start, double end)
    {
        ...
        if (end <= start)
            return null;
        else if (start < 0.0 || start * SAMPLING_RATE > audio.length)
            return null;
        else if (end < 0.0 || end * SAMPLING_RATE > audio.length)
            return null;
        else
        {
            length = (int) (end - start) * SAMPLING_RATE;
            // declare the result array
            double [] result = new double[length];
            // get audio data from the specified range and store in result array
            for (int i = 0; i < length; i++)
                result[i] = audio[(int) (start * SAMPLING_RATE) + i];
            return result; 
        }
    }
    ...
    public static double [] audioPad(double [] audio, double before, double after)
    {
        ...
        else
        {
            ...
            int length = (int) (before + audio.length + after);
            // declare the result array
            double [] result = new double[length];
            // fill the new array
            for (int i = 0; i < length; i++)
                ...
                if (i < before)
                    result[i] = result[i];
            // add original audio
                else if (i < before + audio.length)
                    result[i] = audio[(int) (i - before)];
            // pad after
                else if (i < length)
                    result[i] = result[i];
            return result;   
    ...
    public static double [] audioEcho(double[] audio, double seconds, double factor)
    {
        if (seconds < 0.0)
            return null;
        else
        {
            int j;
            int samples = (int) (seconds * SAMPLING_RATE);
            double [] result = new double[audio.length];
            for (int i = 0; i < audio.length; i++)
            {
                if (i - samples < 0)
                    j = (int) (audio.length - 1 + i - samples);
                else
                    j = (int) (i - samples);
                result[i] = audio[i] + (audio[j] * factor);                
            }
            return result;
        }
    }
    ...
    {    
        ...
        else if (effect.equals("pad"))
        {
            ...
            result = audioPad(audio, before, after);
        }
        else if (effect.equals("echo"))
        {
            ...
            result = audioEcho(audio, seconds, factor);
        }
        else
        {
